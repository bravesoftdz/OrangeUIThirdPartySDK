...
- (NSString *) getSmsPhoneNumber
{
    ...
    const char *phoneNumber = ssl_get_sms_phonenumber(buffer, 15);
    if (phoneNumber == NULL) {
        NSLog(@"get sms phonenumber is null");
        return @"";
    }
    NSString *str = [[NSString alloc] initWithUTF8String:phoneNumber];
    free(buffer);
    return str;
}
...
{
    char* buffer = (char *)malloc(sizeof(char)*15);
    const char *countDown = ssl_get_sms_countdown(buffer,15);
    if (countDown == NULL) {
        NSLog(@"get sms countdown is null");
        return @"";
    }
    NSString  *str = [[NSString alloc] initWithUTF8String:countDown];
    free(buffer);
    return str;
}
...
- (NSString *) getSdkVersion
{
    ...
    const char *version = ssl_get_version();
    if (version == NULL) {
        NSLog(@"current version is null");
        return @"";
    }
    NSString *str = [[NSString alloc] initWithUTF8String:version];
    free(buffer);
    return str;
...
- (NSString *) getVpnError
{
    //NSString *str = [[NSString alloc] initWithUTF8String:get_err()];

    NSString *str = [[NSString alloc] initWithUTF8String:ssl_vpn_get_err()];    
    return str;
