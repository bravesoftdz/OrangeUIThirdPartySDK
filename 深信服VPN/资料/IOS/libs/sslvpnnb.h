...
extern "C"
{
     ...
     **/
    const char* ssl_vpn_query_hardid(char *buffer, int buffer_len);
    
    /**
     * @功能 获取短信验证 手机号
     * @return 返回短信验证 手机号
     */
    const char *ssl_get_sms_phonenumber(char *buffer, int buffer_len);
    
    /**
     * @功能 获取短信验证 再次发送倒计时
     * @return 返回短信验证再次发送倒计时
     */
    const char *ssl_get_sms_countdown();
     ...
     * @return 返回session字符串
     */
    int ssl_get_sessionid(char *buffer, int len);
     ...
     *  @param len     长度
     */
    void ssl_set_sessionid(const char *session,int len);
