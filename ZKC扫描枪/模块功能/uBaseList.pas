...
type
  ...
  public
    ...
    ///   </para>
    /// </summary>
    procedure EndUpdate(Force:Boolean=False);virtual;
...
begin
  ...
  begin
    try
      ObjRelease(AObject);
      uFuncCommon.FreeAndNil(AObject);
    except
      on E:Exception do
      begin
...
begin
  ...
  uFuncCommon.FreeAndNil(FSkinObjectChangeManager);

  uFuncCommon.FreeAndNil(FItems);
  inherited;
end;

procedure TBaseList.EndUpdate(Force:Boolean);
begin
  if FSkinObjectChangeManager<>nil then
  begin
    FSkinObjectChangeManager.EndUpdate(Force);
  end;
end;
...
procedure TSkinObjectChangeManager.EndUpdate(Force:Boolean);
begin
